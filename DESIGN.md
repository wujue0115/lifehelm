...
- **The popover stays open after every pick** — clicking `All` or an option toggles the selection but doesn't close the popover, since checking off several values in one session is the whole point (unlike `SelectMenu`, where there's only ever one value to land on before closing makes sense).
- **The trigger shows the same thing `SelectMenu`'s does when there's one clean way to say it, and a count otherwise**: `allLabel` text when everything's checked, a dedicated "none selected" string when nothing is, the single checked option's own `#option`-slotted content when exactly one is (so a lone status/priority/tag filter still reads as a real badge, not text), and `t('list.filterSelectedCount', { count })` ("N selected") once two or more are checked — there's no single badge that can represent "either of these two," so it falls back to a count rather than trying to cram multiple badges into one trigger line.
- **Checkbox checked state is accent-filled**, not the light tint `select-menu`'s own `.option.selected` uses — this is a literal checkbox (the `SwitchToggle`/`btn-primary` "primary action" category under "Accent (user-configurable)"), and several can be checked at once here, unlike a single-select dropdown's one current value.
...
- **The cell's own DOM never swaps.** An earlier version replaced the read-only badge/pills with a bordered `SelectMenu`/`TagsInput` field for the duration of the edit — closer to a native table's "click to edit" affordance, but it meant the row visibly changed shape/style on every click, and briefly turning a pill into an input-styled box read as noisy rather than as "here's a menu." `bare` mode (see `select-menu` above) keeps the trigger permanently as whatever's passed to `#trigger` (the same `StatusBadge`/`PriorityBadge`/`TagPill` the cell always shows) and only the popover is new — the resting cell is pixel-identical whether or not it's ever been clicked.
- **Both components stay mounted continuously and manage their own open/closed popover state**, the same as every other `SelectMenu`/`TagsInput` usage (filters, `ItemDetailView`'s form fields) — there's no per-cell "which field is currently being edited" state in `WorkItemRow` anymore, and so no `autoOpen`/`autoFocus`/`close`-event machinery either. That machinery existed only to work around the DOM-swap approach above (mounting a fresh editor on click, needing to tell it to open/focus itself immediately, needing to hear back when to unmount) — once the trigger persists, there's nothing to coordinate: a plain click handler toggles the popover exactly like a filter dropdown does.
- **`TagsInput`'s `bare` mode moves the entire chip/input editing box into the floating popover**, not just the suggestion list — unlike `SelectMenu` (pick one, commit, close), tags are add/remove-many, so the popover needs live editing UI, not just options. The always-visible trigger only ever shows the committed pill list.
- **The editors' own option lists use the `#option` badge slot** (see `select-menu` above) so a status/priority/tag option looks identical mid-edit to how it reads once committed — the list's own filters use the same slot for the same reason.
- **A newly-created tag registers the same way `ItemDetailView`'s own submit does** — sequentially, awaiting each `ensureTagRegistered` before saving, not `Promise.all`, for the same read-modify-write race reason.
