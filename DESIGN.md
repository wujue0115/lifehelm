...
**Date filter** (`src/components/DateFilter.vue`) — an advanced date-range filter for the same List/Board/Calendar filters row as the status/priority/tag `<select>`s (`src/types/view.ts`'s `FilterConfig`), styled as the same `date-picker-trigger`-spec button opening a popover under it.
